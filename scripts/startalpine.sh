...
proot-distro login alpine --shared-tmp -- /bin/bash -c \
  'mkdir -p /run/dbus && dbus-daemon --system 2>/dev/null; export PULSE_SERVER=127.0.0.1 && export XDG_RUNTIME_DIR=/tmp && export PATH=$PATH:/sbin && su - YourUsername -c "env DISPLAY=:1 GALLIUM_DRIVER=virpipe MESA_GL_VERSION_OVERRIDE=4.0 dbus-launch --exit-with-session xfce4-session"'
